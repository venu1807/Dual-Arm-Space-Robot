...
% t=linspace(0,30,100);
t=0:0.1:30;
[T,Y]=ode45(@space_links_2_pk,t,[0,0,0,0,0,0]);
...
KE=1/2*m0*(dx0^2+dy0^2)  + 1/2*m1*( (dx0-a0/2*sin(th0)-a1/2*sin(th0+th1) )^2 + (dy0+a0/2*cos(th0)+a1/2*cos(th0+th1) )^2) + 1/2*m1*( (dx0-a0/2*sin(th0)-a1*sin(th0+th1) -a2/2*sin(th0+th1+th2) )^2 + (dy0+a0/2*cos(th0)+a1*cos(th0+th1)+a2/2*sin(th0+th1+th2) )^2) + 1/2*Iz0* dth0^2 +1/2*Iz1*(dth0+dth1)^2 +1/2*Iz2*(dth0+dth1+dth2)^2 ;

PE=m0*y0*g + m1*(y0 +a0/2*sin(th0) +a1/2*sin(th0+th1) )*g +m2 *(y0 +a0/2*sin(th0) +a1*sin(th0+th1) +a2/2*sin(th0+th1+th2) )*g ;
...
%     R=[cos(Y(id,3)) -sin(Y(id,3));sin(Y(id,3)) cos(Y(id,3))]';
%     V=B*R;
% 
%    X0(:,id)=V(:,1);
%    Y0(:,id)=V(:,2);
...
%    
%    x1(id,:) = L1*[cos(M(id,1)+Y(id,3)),   cos(M(id,1)+Y(id,3))+cos(M(id,1)+M(id,2)+Y(id,3))];
%    y1(id,:) = L1*[sin(M(id,1)+Y(id,3)),   sin(M(id,1)+Y(id,3))+sin(M(id,1)+M(id,2)+Y(id,3))]; 
%    
%    
%    B=[x1(id,:)' y1(id,:)'];
% %    rx(:,index)=X0(:,id);
...
% plot(t,Y(:,4),'m-o','DisplayName','dx0')
% 
% hold on
% plot(t,Y(:,5),'b*','DisplayName','dy0');
% legend('show')
% 
% figure
% hold on
