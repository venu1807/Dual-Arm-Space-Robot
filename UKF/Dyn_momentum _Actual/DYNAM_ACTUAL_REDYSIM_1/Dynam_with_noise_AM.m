...
th1_1 = Y(:,7);        %Joint position
v0cI = Y(:,8:10);    %CM linear velocity of the base
om0_1 = Y(:,11);       %CM angular velocity of the base
om1_1 = Y(:,14);       %Joint velocity
Px_1  = m(:,1);
Py_1  = m(:,2);
Lz_1  = m(:,6);
...
Lz  = Lz_1 +(np/100)*range(Py_1)*randn(len_t,1);
...
%         reg_x = [reg_x; l0x l1x; l0y l1y];
          ...
          reg_x = [reg_x; Beta0 Beta1];
          
% %       reg_x = [l0x l1x; l0y l1y];
%     
        Bx = [Bx; Lz(i)-(C0(i,3)+C1(i,3))];
%       Bx = [Px(i); Py(i)]
end
A=reg_x;
B=Bx;
