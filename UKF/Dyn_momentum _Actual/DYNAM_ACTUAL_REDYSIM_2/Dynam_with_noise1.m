...
th0_1 = Y(:,4);        %CM angular position of the base
th1_1 = Y(:,7);        %Joint1 position
th2_1 = Y(:,8);        %Joint2 position
v0cI  = Y(:,9:11);    %CM linear velocity of the base
om0_1 = Y(:,12);       %CM angular velocity of the base
om1_1 = Y(:,15);       %Joint1 velocity
om2_1 = Y(:,16);       %Joint2 velocity
...
th2 = th2_1+(np/100)*range(th2_1)*randn(len_t,1);

om0 = om0_1+(np/100)*range(om0_1)*randn(len_t,1);

om1 = om1_1+(np/100)*range(om1_1)*randn(len_t,1);

om2 = om2_1+(np/100)*range(om2_1)*randn(len_t,1);
...
for i=1:len_t
    ...
    RP2(:,:,i) = [cos(th2(i)) -sin(th2(i)) 0
                  sin(th2(i))  cos(th2(i)) 0
                  0              0         1];         
...
for i=1:1:len_t
	    R0I = RIP(:,:,i);
        R1I = RIP(:,:,i)*RP1(:,:,i);
        R2I = RIP(:,:,i)*RP1(:,:,i)*RP2(:,:,i);
        
	    r1JI(i,:) = rpI(i,:) + rp1JI(i,:);
        
        r2JI(i,:) = rpI(i,:) + rp1JI(i,:) + r1JI(i,:);
        ...
        Va2I(i,:) = Va1I(i,:) - cross([0,0,om2(i)], r2JI(i,:)) + cross([0,0,OMGa2I(i)],r2JI(i,:));
        ...
        l2x = [Va2I(i,1) -OMGa2I(i)*R2I(2,2) -OMGa2I(i)*R2I(2,1)];
        l2y = [Va2I(i,2)  OMGa2I(i)*R2I(1,2)  OMGa2I(i)*R2I(1,1)];
        
    
        reg_x = [reg_x; l0x l1x l2x; l0y l1y l2y];
...
G1=[A(:,1) A(:,2) A(:,3) A(:,5) A(:,6) A(:,7) A(:,8) A(:,9)];
