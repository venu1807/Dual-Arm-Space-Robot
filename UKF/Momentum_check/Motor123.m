...
%     end
    ...
    xdot = [-Ra/L*xpost(1) + xpost(3)*lambda/L*sin(xpost(4)) + ua/L;
            -Ra/L*xpost(2) - xpost(3)*lambda/L*cos(xpost(4)) + ub/L;
            -3/2*lambda/J*x(1)*sin(xpost(4)) + ...
            3/2*lambda/J*xpost(2)*cos(xpost(4)) - B/J*xpost(3);
            xpost(3)];
    xdot = xdot + xdotNoise .* randn(4,1);
    xpriori = xdot * dt; % rectangular integration
%     xpost(4) = mod(x(4), 2*pi); % keep the angle between 0 and 2*pi
    
    A = [-Ra/L 0 lambda/L*sin(xpost(4)) xpost(3)*lambda/L*cos(xpost(4));
        0 -Ra/L -lambda/L*cos(xpost(4)) xpost(3)*lambda/L*sin(xpost(4));
        -3/2*lambda/J*sin(xpost(4)) 3/2*lambda/J*cos(xpost(4)) -B/J ...
        -3/2*lambda/J*(xpost(1)*cos(xpost(4))+xpost(2)*sin(xpost(4)));
        0 0 1 0];
