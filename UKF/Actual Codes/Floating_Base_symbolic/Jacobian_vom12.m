...
Qr=sym(zeros(3,3,n));
Im=sym(zeros(n-1,n-1));
Ibm=sym(zeros(n-1,6));

Dxx=sym(zeros(n,1));Dyy=sym(zeros(n,1));Dzz=sym(zeros(n,1));
Dxy=sym(zeros(n,1));Dyz=sym(zeros(n,1));Dzx=sym(zeros(n,1));
Ixx=sym(zeros(n,1));Iyy=sym(zeros(n,1));Izz=sym(zeros(n,1));
Ixy=sym(zeros(n,1));Iyz=sym(zeros(n,1));Izx=sym(zeros(n,1));
...
H=-inv(Ib)*Ibm;
KL=jacobian(H(3:6,:),[phi1,th1,si1]);
