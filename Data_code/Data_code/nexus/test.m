...
% Program options
% command = 'cd C:\Users\Phani\Dropbox\DH\RobotModel\Material and papers\Inverse Optimal control\Data_code\Forceplate & BertecExample.exe -t 2 -a -z -p &';
% dos(command);
% 
% cd C:\Users\Phani\Dropbox\DH\RobotModel\Material' and papers'\Inverse' Optimal control'\Data_code\nexus;

diary off;
clc;
clear all;
close all;
diary('Apr_12_walk_2.txt')
diary on;

% t = tcpip('localhost', 8888, 'NetworkRole', 'client');
% fopen(t);
% pause(0.001);
% dt = 'r';
...
while ishandle( MessageBox )
 ...
 % fprintf( 'Waiting for new frame...' );
  ...
  if(AdaptBool(Output_GetTimecode.FieldFlag) == 1)
...
%                          AdaptBool( Output_GetSegmentLocalRotationEulerXYZ.Occluded ) );
    ...
    for MarkerIndex = 1:MarkerCount
      ...
      pts(:,MarkerIndex) = Output_GetMarkerGlobalTranslation;
...
%     A = [pts(2).Translation(1),pts(2).Translation(2),pts(2).Translation(3)];
%     B = [pts(3).Translation(1),pts(3).Translation(2),pts(3).Translation(3)];
%     C = [pts(4).Translation(1),pts(4).Translation(2),pts(4).Translation(3)];
%     
%     
%     v21 = [A(1)-B(1),A(2)-B(2),A(3)-B(3)];
%     v23 = [C(1)-B(1),C(2)-B(2),C(3)-B(3)];
%     ang = atan2d(norm(cross(v21,v23)),dot(v21,v23));
%     link_2(180-ang);
% %     figure(2);
%     plot(count,ang,'r.');
%     hold on;
...
%   data = sprintf('%s',data);
%   f_data = strread(data,'%f');
%   fprintf('Fx: %f\nFy: %f\nFz: %f\nMx: %f\nMy: %f\nMz: %f\n',f_data(1),f_data(2),f_data(3),f_data(4),f_data(5),f_data(6));
  %%----------------- 
  
  % Get the unlabeled markers
  UnlabeledMarkerCount = MyClient.GetUnlabeledMarkerCount().MarkerCount;
  if UnlabeledMarkerCount
