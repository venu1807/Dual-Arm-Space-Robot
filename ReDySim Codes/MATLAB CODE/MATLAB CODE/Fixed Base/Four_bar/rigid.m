...
m0=500;m1=.5;m2=.5;m3=.5;         %l0=1;l1=1;l2=1;l3=1; 
...
thin=[pi/2; 90*pi/180; (90)*pi/180];%60
thf=[-pi/4; 90*pi/180; (90)*pi/180];
...
alp=.8*eye(2,2)
beta=.005*eye(2,2)
phi=[l1*cos(th1)- l2*cos(th2)-l3*cos(th2 + th3) 
    l1*sin(th1)- l2*sin(th2)-l3*sin(th2 + th3)]                                                                                

phidt =[ dth2*(l3*sin(th2 + th3) + l2*sin(th2)) - dth1*l1*sin(th1) + dth3*l3*sin(th2 + th3)
 dth1*l1*cos(th1) - dth2*(l3*cos(th2 + th3) + l2*cos(th2)) - dth3*l3*cos(th2 + th3)]
...
%  
 ...
 F2=(J.'*[lmdx;lmdy])
 F3=[0;0;0;-Jdtdq]
 F4=[0;0;0;(2*alp*phidt)-(beta*beta*phi)]
 Ff=F1+[F2;0;0]+F3%+F4
 ddq=Minv*Ff
% 
% L1=-(inv(J*Iinv*J.'))
