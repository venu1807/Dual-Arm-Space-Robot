...
vef=(veff-vefi)*sin(pi*t/2*Tf);

J1=zeros(3,2);
J2 =[ - l1*sin(th0 + th1) - lc0*sin(th0) - l2*sin(th0 + th1 + th2), - l1*sin(th0 + th1) - l2*sin(th0 + th1 + th2), -l2*sin(th0 + th1 + th2)
   l1*cos(th0 + th1) + lc0*cos(th0) + l2*cos(th0 + th1 + th2),   l1*cos(th0 + th1) + l2*cos(th0 + th1 + th2),  l2*cos(th0 + th1 + th2)];
J3=ones(1,3);
 J(:,1:2)=J1;
 J(2:3,3:5)=J2;
 J(1,3:5)=J3;
...
ddx0=ddq(1); ddy0=ddq(2); ddth0=ddq(3); ddth1=ddq(4); ddth2=ddq(5);
 
dy = zeros(10,1);
dy(1)=dx0; dy(2)=dy0; dy(3)=dth0; dy(4)=dth1; dy(5)=dth2;dy(6)=ddx0; dy(7)=ddy0; dy(8)=ddth0; dy(9)=ddth1; dy(10)=ddth2;
