...
fill([x02 x03 x04 x05 x06 x07],[y02 y03 y04 y05 y06 y07],'y')
hold on
...
circ = fncmb(circle,[.5 0;0 .5]);
target=fncmb(fncmb(circ), [2;-2] );
...
%fnplt(circle), axis square
ellipse = fncmb(circle,[.5 0;0 .5]);
Ctx=1.5*x15-.5*x14;Cty=1.5*y15-.5*y14;
target=fncmb(fncmb(ellipse), [Ctx;Cty] );
hold on, fnplt(target), hold off 
end
hold off
